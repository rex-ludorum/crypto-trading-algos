...
void processTradesWithOnlineAlgs(const cl::CommandQueue &queue,
																 const cl::Kernel &kernel,
																 vector<tradeWithoutDate> &tradesWithoutDates,
																 const vector<combo> &comboVec,
																 const cl::Buffer &inputTrades,
																 const cl::Buffer &inputSize) {
	size_t currIdx = 0;
	static size_t globalIdx = 0;
...
int main(int argc, char *argv[]) {
	...
	if (listTrades) {
		...
		MAX_TOTAL_TRADES, entryAndExit{}); entriesAndExitsBuf = cl::Buffer(context,
		CL_MEM_WRITE_ONLY | CL_MEM_HOST_READ_ONLY | CL_MEM_COPY_HOST_PTR,
		comboVec.size() * sizeof(entryAndExit) * MAX_TOTAL_TRADES,
		&entriesAndExitsVec[0], &err); if (err != CL_SUCCESS) { cout << "Error for
		entriesAndExitsBuf: " << err << endl; return 1;
		}
		*/
	...
	if (writeResults) {
		ofstream outFile;
		if (isBTC)
			outFile.open("resultsTrendBTC");
		else
			outFile.open("resultsTrendETH");
		if (outFile.is_open()) {
			int maxElementIdx =
