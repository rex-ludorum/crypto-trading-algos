...
void saveSnapshot(size_t index, const vector<entry> &entriesVec,
...
int processTradesWithIndicators(
		...
		const cl::Buffer &entries, const cl::Buffer &tradeRecords,
		const cl::Buffer &positionDatas, const cl::Buffer &numTradesInIntervalBuf,
	...
	if (snapshotting) {
		...
		saveSnapshot(globalIdx, entriesVec, tradeRecordsVec, positionDatasVec,
...
void processTradesWithOnlineAlgs(
		...
		const cl::Buffer &inputTrades, const cl::Buffer &inputSize,
		const cl::Buffer &entries, const cl::Buffer &tradeRecords,
		const cl::Buffer &positionDatas, const cl::Buffer &drawdownsBuf,
...
int main(int argc, char *argv[]) {
	...
	if (err != CL_SUCCESS) {
		cout << "Error for trendKernel setArg 0: " << err << endl;
	}
	err = trendKernel.setArg(1, inputTrades);
	if (err != CL_SUCCESS) {
		cout << "Error for trendKernel setArg 1: " << err << endl;
	}
	err = trendKernel.setArg(2, inputCombos);
	if (err != CL_SUCCESS) {
		cout << "Error for trendKernel setArg 2: " << err << endl;
	}
	...
	if (listTrades) {
		err = trendKernel.setArg(6, drawdownsBuf);
		if (err != CL_SUCCESS) {
			cout << "Error for trendKernel setArg 6: " << err << endl;
		}
		err = trendKernel.setArg(7, drawdownLengthsBuf);
		if (err != CL_SUCCESS) {
			cout << "Error for trendKernel setArg 7: " << err << endl;
		}
		err = trendKernel.setArg(8, lossStreaksBuf);
		if (err != CL_SUCCESS) {
			cout << "Error for trendKernel setArg 8: " << err << endl;
		}
		err = trendKernel.setArg(9, tradeDurationsBuf);
		if (err != CL_SUCCESS) {
			cout << "Error for trendKernel setArg 9: " << err << endl;
		}
		err = trendKernel.setArg(10, monthlyReturnsBuf);
		if (err != CL_SUCCESS) {
			cout << "Error for trendKernel setArg 10: " << err << endl;
		}
		err = trendKernel.setArg(11, winsBuf);
		if (err != CL_SUCCESS) {
			cout << "Error for trendKernel setArg 11: " << err << endl;
		}
		err = trendKernel.setArg(12, lossesBuf);
		if (err != CL_SUCCESS) {
			cout << "Error for trendKernel setArg 12: " << err << endl;
		}
	} else {
		...
		if (myFile.is_open()) {
			...
			while (getline(myFile, line)) {
				...
				*/
				...
				if (!snapshotting || (snapshotting && currIdx >= globalIdx)) {
#ifdef DEBUG
				...
				if (tradesWithoutDates.size() == TRADE_CHUNK) {
					...
					if (listTrades)
						processTradesWithOnlineAlgs(
								queue, trendKernel, tradesWithoutDates, comboVec, inputTrades,
								inputSize, entries, tradeRecords, positionDatas, drawdownsBuf,
								drawdownLengthsBuf, lossStreaksBuf, tradeDurationsBuf,
								monthlyReturnsBuf, winsBuf, lossesBuf, entriesVec,
						...
						maxTradesPerInterval =
								max(maxTradesPerInterval,
										processTradesWithIndicators(
												queue, trendKernel, tradesWithoutDates, comboVec,
												inputTrades, inputSize, entries, tradeRecords,
												positionDatas, numTradesInIntervalBuf, entriesVec,
												tradeRecordsVec, positionDatasVec,
	...
	if (!justProcessed) {
		...
		if (tradesWithoutDates.size() > 0) {
			if (firstRun)
				firstRun = false;
			if (listTrades)
				// processTradesWithListing(queue, trendKernel, tradesWithoutDates,
				// comboVec, 												 inputTrades, inputSize,
				// twBetweenRunData, 												 positionDatas,
				// numTradesInIntervalBuf,
				//												 allTrades);
				// processTradesWithListingAndIndicators(
				// 		queue, trendKernel, tradesWithoutDates, indicators, tws, comboVec,
				...
				maxTradesPerInterval = max(
						maxTradesPerInterval,
						// processTrades(queue, trendKernel, tradesWithoutDates,
						// comboVec, 							inputTrades, inputSize, twBetweenRunData,
						// 							positionDatas, numTradesInIntervalBuf));
						processTradesWithIndicators(
								queue, trendKernel, tradesWithoutDates, comboVec, inputTrades,
								inputSize, entries, tradeRecords, positionDatas,
								numTradesInIntervalBuf, entriesVec, tradeRecordsVec,
								positionDatasVec, numTradesInIntervalVec, snapshotting));
		} else
	...
	string resultsFilename = "resultsTrend";
	if (isBTC)
		resultsFilename += "BTC";
	else
		resultsFilename += "ETH";
