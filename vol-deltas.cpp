...
vector< vector<double> > allDeltas(NUM_WINDOWS);
...
void *performWork(void *arguments){
	...
	bool startCollectingDeltas = false;
	...
	for (size_t i = 0; i < qtys.size(); i++) {
		...
		if (startCollectingDeltas) {
			deltas.push_back(buyVol - sellVol);
		}
	}
	sort(deltas.begin(), deltas.end());
	allDeltas[index] = deltas;
