...
int main(int argc, char* argv[]) {
	...
	*/
	...
	cl::Kernel volTrendKernel;
	if (listTrades) {
		volTrendKernel = cl::Kernel(program, "volTrendTraderWithTrades", &err);
	} else {
		volTrendKernel = cl::Kernel(program, "volTrendTrader", &err);
	}
	err = volTrendKernel.setArg(0, inputSize);
	if (err != CL_SUCCESS) {
	...
	err = volTrendKernel.setArg(3, entries);
	if (err != CL_SUCCESS) {
		cout << "Error for volTrendKernel setArg 3: " << err << endl;
	}
	err = volTrendKernel.setArg(4, tradeRecords);
	if (err != CL_SUCCESS) {
		cout << "Error for volTrendKernel setArg 4: " << err << endl;
	}
	err = volTrendKernel.setArg(5, positionDatas);
	if (err != CL_SUCCESS) {
		cout << "Error for volTrendKernel setArg 5: " << err << endl;
	}
	err = volTrendKernel.setArg(6, twBetweenRunData);
	if (err != CL_SUCCESS) {
	...
	if (listTrades) {
		...
		err = volTrendKernel.setArg(7, numTradesInIntervalBuf);
		if (err != CL_SUCCESS) {
			cout << "Error for volTrendKernel setArg 7: " << err << endl;
		}
	}
