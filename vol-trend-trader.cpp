...
void processTradesWithOnlineAlgs(
		...
		const cl::Buffer &monthlyReturnsBuf, const cl::Buffer &winsBuf,
		const cl::Buffer &lossesBuf, vector<entry> &entriesVec,
		vector<tradeRecord> &tradeRecordsVec,
...
int main(int argc, char *argv[]) {
	...
	if (listTrades) {
		...
		// &err);
		if (futures)
			kernel = cl::Kernel(program, "volTrendTraderFuturesWithOnlineAlgs", &err);
		else
			kernel = cl::Kernel(program, "volTrendTraderWithOnlineAlgs", &err);
	} else {
		if (futures)
			kernel = cl::Kernel(program, "volTrendTraderFuturesWithIndicators", &err);
		else
			kernel = cl::Kernel(program, "volTrendTraderWithIndicators", &err);
	}
	if (err != CL_SUCCESS) {
	...
	if (listTrades) {
		...
		err = kernel.setArg(12, winsBuf);
		if (err != CL_SUCCESS) {
			cout << "Error for kernel setArg 11: " << err << endl;
		}
		err = kernel.setArg(13, lossesBuf);
		if (err != CL_SUCCESS) {
			cout << "Error for kernel setArg 12: " << err << endl;
		}
	} else {
				...
				if (tradesWithoutDates.size() - newStart == TRADE_CHUNK) {
					...
					if (listTrades)
						processTradesWithOnlineAlgs(
								...
								entriesVec, tradeRecordsVec, positionDatasVec, drawdownsVec,
	...
	string resultsFilename = "resultsVolTrend";
	if (isBTC)
		resultsFilename += "BTC";
	else
		resultsFilename += "ETH";
